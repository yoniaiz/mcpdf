...
## Step 3: Read Context for Commit Message

Read these files (using the active phase):
- `context/{phase}/PROGRESS.md` - Get the task that was completed
...
## Step 4: Review Changes One More Time
...
git add src/
git add tests/
git add context/{phase}/PROGRESS.md
# etc.
\`\`\`
...
**Scope examples:** \`pdf\`, \`mcp\`, \`tools\`, \`server\`, \`core\`, \`static-forms\`
...
## Step 10: Update PROGRESS.md (if not done)

Ensure `context/{phase}/PROGRESS.md` reflects the completion:
...
## Task Status
- Task X.Y marked complete in context/{phase}/PROGRESS.md
- Change log updated
...
**Next steps:**
- Review context/{phase}/PROGRESS.md for the next pending task
...
- ❌ Forget to include phase in commit message
